...
- `Compose 手势实验区` 提供 pointerInput tap、drag、Button click。
...
```text
你能解释为什么父容器拦截 MOVE 后，子 View 的手势可能被取消。
```
...
```text
你能把 Compose 手势、传统 View 分发和主线程响应放进同一张输入链路地图里。
```
...
- `InputLabStore.kt`：记录 MotionEvent、Compose 手势、拦截开关和 Logcat 日志。
- `InputLabScreen.kt`：展示输入观察页面、原生 View 实验区、Compose 手势区和事件轨迹。
