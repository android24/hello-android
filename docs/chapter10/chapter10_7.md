...
你的 App 想启动页面、发通知、查安装包，就像去系统办事大厅提交申请。

但办事大厅不在你的 App 进程里。你不能直接冲进系统服务的办公室翻资料，只能通过一条受管控的通道提交请求、等待处理、拿回结果。

Binder 就是这条通道。它让“跨楼办事”看起来像在本地窗口递了一张表。
...
请判断下面哪些操作需要去系统办事大厅，哪些只是 App 自己在房间里做计算。
