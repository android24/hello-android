...
但在系统后台，这一下像按下了发车按钮：Launcher 发出请求，系统服务检查线路，Zygote 准备新车厢，ActivityThread 接过调度牌，最后你的 `Activity.onCreate()` 才真正登场。

本节要追的不是一个方法，而是一趟从桌面到页面的启动列车。
...
请把下面这条启动线路补全。你不需要写出所有站点，先把关键换乘点找出来。
