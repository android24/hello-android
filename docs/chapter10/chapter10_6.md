...
点击事件、生命周期回调、UI 更新、绘制任务都会被放到这条传送带上。`Handler` 负责投递任务，`MessageQueue` 负责排队，`Looper` 负责一件件取出来处理。

如果你把一个超重包裹长时间压在传送带上，后面的所有任务都会跟着等。这就是卡顿和 ANR 让人头疼的地方。
...
请把一次 `Handler.post()` 想象成把一个小任务放上传送带。它最终要被谁取走，又会在哪里执行？
